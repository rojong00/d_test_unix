...
int main(int argc, char* argv[])
{
    uint8_t aucCmdBuf[MAX] = {0xFD, 0x08, 0x0018, 
        0x20, 0x03, 0x81,
        0x01, 0x01, 0x07};
    unsigned char ucTotalPacketLength = 8; // => 8
    // FD : 1B, Len(except FD and Len) : 1B, MsgID : 2B, 
    // Category : 1B, Module : 1B, ActionItem : 1B,
    // No.params : 1B, Len.param : 1B, param : nB(1N)
    //
    // CRC16 = 0x3611
    //
    int i = 0;
    ...
    if (strlen(argv[1]) != 20)
    {
        printf("INPUT IS WRONG : !20\n");
        return 1;
    }
    strncpy(code, argv[1], strlen(argv[1]));
    printf("code is %s\n", code);
    code[0] = 253;
    printf("%02X ", code[0]);
    for (i = 1; i < 10; i++)
    {
        code[i] = (code[i*2]-'0')*16 + code[i*2+1]-'0';
        printf("%02X ", code[i]);
    }
    code[i] = '\0';
    printf("\n");
